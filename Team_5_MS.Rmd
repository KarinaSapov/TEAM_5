...
px
pxfiles(px)
pxget(px, grep("mzid", pxfiles(px)))
pxget(px, grep("mzML", pxfiles(px)))
f <- pxget(px, grep("mzid", pxfiles(px)))
mzML <- pxget(px, grep("mzML", pxfiles(px)))
pxurl(px)
ident()
proteomics()
ident()
...
sp <- Spectra(mzML)
sp
obj <- PSM(f, spectrum = NA, peptide = NA, protein = NA, decoy = NA, rank = NA, score = NA, fdr = NA, parser = c("mzR", "mzID"), BPPARAM = SerialParam())
